...
(function (global, factory) {
  ...
  */
  ...
  var updateSlides = function updateSlides() {
    ...
    var offsetBefore = params.slidSymmetryfsetBefore;
    if (typeof offsetBefore === 'function') {
      offsetBefore = params.slidSymmetryfsetBefore.call(swiper);
    }

    var offsetAfter = params.slidSymmetryfsetAfter;
    if (typeof offsetAfter === 'function') {
      offsetAfter = params.slidSymmetryfsetAfter.call(swiper);
    }
  ...
  var updateSlidSymmetryfset = function updateSlidSymmetryfset() {
    var swiper = this;
  ...
  var update = {
    ...
    updateSlidSymmetryfset: updateSlidSymmetryfset,
  ...
  var defaults = {
    ...
    slidSymmetryfsetBefore: 0, // in px
    slidSymmetryfsetAfter: 0, // in px
  ...
  base.ammo = function () {
    ...
    */
    var _each = function _each(elements, callback) {
      Object.keys(elements).forEach(function (key, i) {
    ...
    * @returns {*}
    */
    var parseToType = function parseToType(val) {
      if (!isStr(val)) {
        return undefined;
      }
    ...
    */
    var getElementData = function getElementData($el) {
      ...
      eachKey(dataSet, function (val, key) {
        return options[key] = parseToType(val);
      });
      return options;
    };
    ...
    * @param excludes
    */
    var runMethodsInParallel = function runMethodsInParallel(obj) {
      var excludes = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];

      var excludedMethods = isArr(excludes) ? excludes : [];

      eachKey(obj, function (prop, key) {
    ...
    * @param args
    */
    var scrollSpy = function scrollSpy(args) {

      // validate callbacks
      var isValid = (typeof args === "undefined" ? "undefined" : _typeof(args)) === 'object' && typeof args.offset === 'number' && _typeof(args.callbacks) === 'object' && typeof args.callbacks.onBefore === 'function' && typeof args.callbacks.onAfter === 'function';
    ...
    */
    return {
      ...
      runMethodsInParallel: runMethodsInParallel,
      scrollSpy: scrollSpy
    };
  }();
})(window);
...
symmetry.addNode('core', 'api', function () {
  var _window$symmetrySetti = window.symmetrySettings,
      ...
      ajaxToken = _window$symmetrySetti.ajaxToken;
  ...
  * @returns object
  */
  var getAjaxOptions = function getAjaxOptions(_ref) {
  ...
  * @param callback
  */
  var getInterfaceSettings = function getInterfaceSettings(_ref2) {
...
symmetry.addNode('core', 'globalEvents', function () {
  var eventPrefix = 'symmetry';
  var ACTIVE_PROJECT_INDEX_CHANGE = 'ACTIVE_PROJECT_INDEX_CHANGE';
  ...
  */
  ...
  return {
    events: { ACTIVE_PROJECT_INDEX_CHANGE: ACTIVE_PROJECT_INDEX_CHANGE },
    dispatchChangeActiveProjectIndex: function dispatchChangeActiveProjectIndex() {
      dispatchEvent(ACTIVE_PROJECT_INDEX_CHANGE);
      return this;
    },
    interceptChangeActiveProjectIndex: function interceptChangeActiveProjectIndex(callback) {
      interceptEvent(ACTIVE_PROJECT_INDEX_CHANGE, { callback: callback });
      return this;
    }
  };
});
...
symmetry.addNode('core', 'manager', function () {
  var app = symmetry;
  var props = {};
  var store = { processed: [] };
  ...
  }).node('runCommon', function () {
    var common = app.getNodes('common');
    ammo.runMethodsInParallel(common);
  }).node('runAdmin', function () {
    var admin = app.getNodes('admin');
    ammo.runMethodsInParallel(admin);
  }).node('runComponents', function () {
    var components = app.getNodes('components');

    ammo.selectAll('[data-component]').each(function ($component) {
  ...
  return {
    runCommon: function runCommon() {
      module.callNode('actions', 'runCommon');
      return this;
    },
    runAdmin: function runAdmin() {
      var _app$getNodes = app.getNodes('admin'),
          admin = _app$getNodes.admin;
...
symmetry.addNode("components", "project", function ($component, component) {
  "use strict";

  // const component = ammo.app().schema("component");

  component.configure("actions").node("init", function () {
...
symmetry.addNode('components', 'projects-slider', function ($component) {
  ...
  component.configure('renderers').node('animateMoveToNextProject', function (scrollTop) {
    ...
    var activeProject = $component.querySelector('.slider-item.active');
    var activeProjectIndex = -1;
    var nextActiveProject = '';
    ...
    var $nextActiveProject = $(nextActiveProject);
    var nextActiveProjectOffset = $nextActiveProject.offset().top;
    var scrollTop = activeProjectIndex > 0 ? nextActiveProjectOffset : nextActiveProjectOffset;
  ...
  component.configure('actions').node('randomGradient', function ($target, multiplier) {
    ...
    questionOffsets.forEach(function (questionOffset, index) {
      ...
      if (index + 1 < questionOffsets.length) {
        var nextQuestionOffset = questionOffsets[index + 1];
  ...
  }).node('getOffsets', function () {
    var offsets = [];
    var projects = selectAll('.slider-item', $component);
    projects.each(function (project) {
      var $project = $(project);
      var offset = parseInt($project.offset().top + $project.height(), 10);
      offsets.push(offset);
    });
    return offsets;
  }).node('init', function () {
    var _component$getNodes2 = component.getNodes(),
        events = _component$getNodes2.events,
        renderers = _component$getNodes2.renderers,
        actions = _component$getNodes2.actions;
